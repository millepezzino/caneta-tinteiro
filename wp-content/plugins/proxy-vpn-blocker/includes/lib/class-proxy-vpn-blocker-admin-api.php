...
class proxy_vpn_blocker_Admin_API {
	...
	public function display_field ( $data = array(), $post = false, $echo = true ) {
		...
		switch( $field['type'] ) {
            ...
            case 'select_country_multi':
				$html .= '<select class="form-control chosen-select" data-placeholder="' . esc_attr( $field['placeholder'] ) . '" name="' . esc_attr( $option_name ) . '[]" id="' . esc_attr( $field['id'] ) . '" multiple autocomplete="off">';
				foreach ( $field['options'] as $k => $v ) {
					$selected = false;
					if ( in_array( $k, (array) $data ) ) {
						$selected = true;
	...
	public function validate_field_checkbox ( $data = '', $type = 'checkbox' ) {
		...
		switch( $type ) {
			case 'checkbox': $data = esc_attr( $data ); break;
		}
	...
	public function validate_field_apikey ( $data = '', $type = 'apikey' ) {
		...
		switch( $type ) {
			case 'apikey': $data = esc_attr( $data ); break;
		}
